...
.. py:function:: run_mclust_partition(banksy_dict: dict, partition_seed: int = 1234, annotations = None, num_labels: int = None, **kwargs  ) -> dict:
   ...
   :param Optional[str, None] annotations:  Key (``str``) to access manual annotations if provided provided under ``adata.obsm[{annotation}]``, otherwise set ``annotations = None``. If so, we also compute the ARIs from each parameter setting under ``results_df[param_name]['ari']`` 
