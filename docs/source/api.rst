...
.. py:function:: initialize_banksy(adata: anndata.AnnData, coord_keys: Tuple[str],   num_neighbours: int = 15,   nbr_weight_decay: str = 'scaled_gaussian',   max_m: int = 1,  plt_edge_hist: bool = True, plt_nbr_weights: bool = True,  plt_agf_angles: bool = False,  plt_theta: bool = True ) -> dict: 
   ...
   :param AnnData adata: ``AnnData``  containing the data matrix.
   ...
   :return: ``banksy_dict``: A dictionary  containing the graph of weights obtained from the neigbhourhood weight decay function. The graph data can be accessed via ``banksy['weights']``.
   :rtype: ``dict``
...
.. py:function:: generate_banksy_matrix(adata: anndata.AnnData, banksy_dict: dict, lambda_list: List[float], max_m: int, plot_std: bool = False, save_matrix: bool = False, save_folder: str = './data', variance_balance: bool = False, verbose: bool = True) -> Tuple[dict, np.ndarray]:
   ...
   Creates the banksy matrices with the set hyperparameters given. Stores the computed banksy matrices in the ``banksy_dict`` , also returns the *last* ``banksy matrix`` that was computed.

   :param AnnData adata: ``AnnData``  containing the data matrix.
   :param dict banksy_dict: The banksy_dict  generated from ``initialize_banksy`` function. Note that this function also returns the same ``banksy_dict`` , it appends computed ``banksy_matrix`` for each hyperparameter under ``banksy_dict[nbr_weight_decay][lambda_param]``.
   ...
   :return: ``banksy_dict``: Updated dictionary  containing computed ``banksy_matrix`` for each hyperparameter under ``banksy_dict[nbr_weight_decay][lambda_param]``.
...
.. py:function:: run_Leiden_partition(banksy_dict: dict, resolutions: List[float], num_nn: int = 50, num_iterations: int = -1, partition_seed: int = 1234, match_labels: bool = True, annotations = None, max_labels: int = None,**kwargs) -> dict:
   ...
   :param dict banksy_dict: The ``banksy_dict``  containing the ``banksy_matrices`` generated from ``embed_banksy`` function. 
   :param Union[List[float], None] resolutions: A list of ``resolution`` parameters that is used for leiden clustering, e.g., ``resolution = [0.2, 0.8]``.  We recommend users to try to adjust resolutions to match the number of clusters that they need. An iterative search for the ``resolution`` that matches the number of ``max_labels`` is conducted if the user sets ``resolution = []`` and ``max_labels`` to their desired cluster number. 
   :param int num_nn: (a.k.a ``k_expr``)  Number of nearest neighbours for Leiden-parition. Also refered to as ``k_expr`` in our manuscript, default = 50.
   ...
   :param Optional[str, None] annotations:  Key to access manual annotations if available under ``adata.obsm[{annotation}]``, otherwise set ``annotations = None``. If so, we also compute the ARIs under ``results_df[param_name]['ari']`` 
...
.. note::
   ...
   Using ``run_Leiden_partition``, the results in ``results_df`` can be accessed via ``param_str`` -  unique id for parameters. i.e.,

   ``param_str = f"{nbr_weight_decay}_pc{pca_dim}_nc{lambda_param}_r{resolution}"`` 
      ...
      ``results_df[param_str] = {``
            ...
            ``"adata": banksy_dict[nbr_weight_decay][lambda_param]["adata"]`` - ``AnnData`` 
      ``}``
...
.. py:function:: run_mclust_partition(banksy_dict: dict, partition_seed: int = 1234, annotations = None, num_labels: int = None, **kwargs  ) -> dict:
   ...
   :param Optional[str, None] annotations:  Key to access manual annotations if provided provided under ``adata.obsm[{annotation}]``, otherwise set ``annotations = None``. If annotations are present, we also compute the ARIs from each parameter setting under ``results_df[param_name]['ari']`` 
...
.. note::
   ...
   Using ``run_mclust_partition``, the results in ``results_df`` can be accessed via ``param_str`` -  unique id for parameters, i.e.,
...
.. py:function:: plot_results(results_df: pd.DataFrame, weights_graph: Union[csc_matrix, csr_matrix], c_map: str,  match_labels: bool, coord_keys: Tuple[str], max_num_labels: int = 20, save_fig: bool = False, save_fullfig: bool = False, save_path: str = None, plot_dot_plot: bool = False, plot_heat_map: bool = False, n_genes: int = 5, main_figsize: Tuple[float, float] = (15, 9),**kwargs) -> None
   ...
   :param int max_num_labels: Number of labels used to match labels (if ``match_labels=True``). Defaults to 20.
   :param bool save_fig: Whether to save the figure containing (only) spatial clusters generated by BANKSY. All figure are saved as ``{nbr_weight_decay}_pc{pca_dim}_nc{lambda_param}_r{resolution}.png``.
...
.. py:function:: run_banksy_multiparam(adata: anndata.AnnData, banksy_dict: dict, lambda_list: List[int], resolutions: List[int], color_list: Union[List, str], max_m: int, filepath: str, key: Tuple[str], match_labels: bool = False, pca_dims: List[int] = [20, ], savefig: bool = True, annotation_key: str = "cluster_name", max_labels: int = None, variance_balance: bool = False, cluster_algorithm: str = 'leiden', partition_seed: int = 1234, add_nonspatial: bool = True, **kwargs) -> None
   ...
   :param AnnData adata: ``AnnData``  containing the data matrix
   :param dict banksy_dict: The banksy_dict  generated from ``initialize_banksy`` function. Note that this function also returns the same ``banksy_dict``, it appends computed ``banksy_matrix`` for each hyperparameter under ``banksy_dict[nbr_weight_decay][lambda_param]``.
   :param List[float] lambda_list: A list of `lambda` parameters that the users can specify, e.g., ``lambda_list = [0.2, 0.8]``. Users can run ``banksy`` with multiple ``lambda`` parameters at once (e.g., setting ``lambda_list = [0.2, 0.4, 0.6, ..]``). We recommend ``lambda_list = [0.2]`` for cell-typing and ``lambda_list = [0.8]`` for domain segemntation. 
   :param List[float] resolutions: Resolution parameter used for ``leiden`` clustering. We recommend users to try to adjust resolutions to match the number of clusters that they need. 
   ...
   :param bool match_labels:  If the match labels options was previously indicated. The output figures will match the clusters generated from BANKSY using different hypeparameters.
   :param List[int] pca_dims: A list of integers which the PCA will reduce to. For example, specifying ``pca_dims = [10,20]`` will generate two set of results - one processed with `10 PCs` and another processed with `20 PCs`. The results can be accessed by retreiving the adata : ``adata = banksy_dict[{nbr_weight_decay}][{lambda_param}][{adata}]``, then the PCA embeddings can be accessed under ``adata.obsm`` via ``pca_embeddings = adata.obsm[reduced_pc_{pca_dim}]``. Defaults to [20]
   :param bool savefig: Whether to save the figure containing (only) spatial clusters generated by BANKSY. All figure are saved via the name ``{nbr_weight_decay}_pc{pca_dim}_nc{lambda_param}_r{resolution}.png``.
...
.. py:function:: pca_umap(banksy_dict: dict,pca_dims: List[int] = [20,], plt_remaining_var: bool = True, add_umap: bool = False, **kwargs) -> Tuple[dict, np.ndarray]
   ...
   :param List[int] pca_dims: A list of integers which the PCA will reduce to. For example, specifying ``pca_dims = [10,20]`` will generate two sets of reduced `pca_embeddings` which can be accessed by first retreiving the adata : ``adata = banksy_dict[{nbr_weight_decay}][{lambda_param}][{adata}]``. Then taking the pca embedding from ``pca_embeddings = adata.obsm[reduced_pc_{pca_dim}]``. Defaults to [20].
   ...
   :return: ``banksy_dict``:  Dictionary containing the graph of weights obtained from the neigbhourhood weight decay function. The graph data can be accessed via ``banksy['weights']``
   :return: ``banksy_matrix``: The last ``banksy_matrix`` generated, useful if the use is simply running one set of parameters.
   :rtype: Tuple(``dict``, ``np.ndarray``)
...
.. py:function:: refine_clusters(adata: anndata.AnnData, results_df: pd.DataFrame, coord_keys: tuple, color_list: list = spagcn_color, savefig: bool = False, output_folder: str = "",  refine_method: str = "once", refine_iterations: int = 1, annotation_key: str = "manual_annotations", num_neigh: int = 6, verbose: bool = False) -> pd.DataFrame:
   ...
   :param AnnData adata:  ``AnnData`` 
   :param Tuple[str] coord_keys: A tuple containing 3 keys to access the `x`, `y` and `xy` coordinates of the cell positions under ``data.obs``. For example, ``coord_keys = ('x','y','xy')``, in which ``adata.obs['x']`` and ``adata.obs['y']`` are 1-D numpy arrays, and ``adata.obs['xy']`` is a 2-D numpy array.
   :param pd.DataFrame results_df: ``DataFrame``  containing the results from ``run_banksy``.
   ...
   :param Optional[int] num_neigh: Number of nearest-neighbours in which refinement is conducted over. By default, we use ``num_neigh = 6``, which is the same as ``SpaGCN``'s original implementation.
   :param Optional[bool] verbose: Whether to print steps conducted during each iteration process.
   :return: ``results_df``: ``DataFrame`` containing the results.
   :rtype: ``pd.DataFrame``
