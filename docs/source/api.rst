...
.. note::
   ...
   ``param_str = f"{nbr_weight_decay}_pc{pca_dim}_nc{lambda_param}_r{resolution}"`` 
      ...
      ``results_df[param_str] = {``
            ...
            ``"adata": banksy_dict[nbr_weight_decay][lambda_param]["adata"]`` - ``AnnData`` object
      ``}``
